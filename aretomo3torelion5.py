...
def create_combined_tomogram_star(tomogram_data_list, output_dir):
    ...
    with open(tomogram_star_path, 'w') as f:
        ...
        for data in tomogram_data_list:
            ...
            tilt_series_star_rel = os.path.join(output_dir, f"{tomo_prefix}.star")
            etomo_directive_rel = os.path.join(output_dir, f"{tomo_prefix}.edf")

            f.write(
                f"{tomo_prefix}_Vol   {data['voltage']:.6f}   {data['cs']:.6f}   {data['amp_contrast']:.6f}   "
...
def create_individual_tilt_series_star(tomogram_data, output_dir):
    ...
    with open(tilt_series_star_path, 'w') as f:
        f.write("# Generated by AreTomo3 to RELION5 converter by P.VdS\n")
        f.write("# Relion star file version 50001\n\n")
        f.write(f"data_{tomo_prefix}_Vol\n\n")
